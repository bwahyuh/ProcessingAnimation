void drawScene5(float duration, float position)
{
  float progress = (frameToSec(counter)-position)/duration;
   ...
   bgCol = colorPalette[4];
       ...
       bgDis5.x++; 
  pushMatrix();
      translate(bgDis5.x, bgDis2.y);
      drawCloud(220, 300, 1, 1);
  popMatrix();
      
  pushMatrix();
      translate(bgDis5.x*2, bgDis2.y);
      drawCloud(70, 100, 1.5, 1.5);
  popMatrix();
  
  pushMatrix();
      translate(bgDis5.x/1.5, bgDis2.y);
  ...
  popMatrix();
    ...
    kakiM1 = sin((norm(counter, 0, 1)/15f)) * 20;
    laki1(jalan5,100, "ngiri");
    ...
    kakiM2 = sin((norm(counter, 0, 1)/15f)) * 20;
    laki2(jalan6,100);
  }if(progress > 0.2){
  textFont(font_cakap);
  textSize(30);
  textAlign(LEFT);
  fill(colorPalette[1]);
  text(textCakap3, width/2 + 300, 300);
  }
}
